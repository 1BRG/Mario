...
BrickBlock::BrickBlock(const float dx, const float dy) : MovebleEnvironment(dx, dy, new Animation *[3]{
        new Animation(BrickTexture, 1, 1),
        new Animation(BrickTexture, 1, 6),
        new Animation(BrickTexture, 1, 6)
    }) {
    state = IDLE;
    health = 2;
